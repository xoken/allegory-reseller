          ...
          script {
            if ((env.BRANCH_NAME).startsWith("release")) {   
              ...
              dir(path: 'allegory-reseller'){
                      ...
                      sh 'docker run -t -d --cidfile /tmp/ubuntu1804.cid -w  /opt/work/allegory-reseller  xoken-nexa/ubuntu18.04 sh'
                      ...
                      sh 'docker exec -w /opt/work/allegory-reseller $(cat /tmp/reseller-ubuntu1804.cid) stack install  --local-bin-path  . '
                      sh 'docker cp $(cat /tmp/reseller-ubuntu1804.cid):/opt/work/allegory-reseller/reseller  . '
                      sh 'rm -f /tmp/reseller-ubuntu1804.cid'
                      sh 'sha256sum ./reseller > Checksum_SHA256'
                      sh 'zip allegory-reseller_"$(basename $(git symbolic-ref HEAD))"_ubuntu1804.zip ./reseller node-config.yaml README.md Checksum_SHA256 LICENSE LICENSE-AGPL LICENSE-OpenBSV '
                    }
              echo '****** Starting Ubuntu20.04 container ******'
              dir(path: 'allegory-reseller'){
                      sh 'rm -f /tmp/reseller-ubuntu2004.cid'
                      sh 'docker run -t -d --cidfile /tmp/ubuntu2004.cid -w  /opt/work/allegory-reseller  xoken-nexa/ubuntu18.04 sh'
                      ...
                      sh 'docker exec -w /opt/work/allegory-reseller $(cat /tmp/reseller-ubuntu2004.cid) stack install  --local-bin-path  . '
                      sh 'docker cp $(cat /tmp/reseller-ubuntu2004.cid):/opt/work/allegory-reseller/reseller  . '
                      sh 'rm -f /tmp/reseller-ubuntu2004.cid'
                      sh 'sha256sum ./reseller > Checksum_SHA256'
                      sh 'zip allegory-reseller_"$(basename $(git symbolic-ref HEAD))"_ubuntu2004.zip ./reseller node-config.yaml README.md Checksum_SHA256 LICENSE LICENSE-AGPL LICENSE-OpenBSV '
                    }
              echo '****** Starting Arch Linux container ******'
              dir(path: 'allegory-reseller'){
                      sh 'rm -f /tmp/reseller-archlinux.cid'
                      sh 'docker run -t -d --cidfile /tmp/archlinux.cid -w  /opt/work/allegory-reseller  xoken-nexa/ubuntu18.04 sh'
                      ...
                      sh 'docker exec -w /opt/work/allegory-reseller $(cat /tmp/reseller-archlinux.cid) env LD_PRELOAD=/usr/lib/libjemalloc.so.2 stack install  --local-bin-path  . '
                      sh 'docker cp $(cat /tmp/reseller-archlinux.cid):/opt/work/allegory-reseller/reseller  . '
                      sh 'rm -f /tmp/reseller-archlinux.cid'
                      sh 'sha256sum ./reseller > Checksum_SHA256'
                      sh 'zip allegory-reseller_"$(basename $(git symbolic-ref HEAD))"_archlinux.zip ./reseller node-config.yaml README.md Checksum_SHA256 LICENSE LICENSE-AGPL LICENSE-OpenBSV '
                    }              
